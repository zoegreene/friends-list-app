...
//[PK] Why make your source files accessible?
app.use("/src", express.static(path.join(__dirname, "src")))
app.use(bodyParser.json())

//[PK] probably put these routes into a router in another file to keep this file small!
// GET /api/friends
...
app.post("/api/friends", async (req, res, next) => {
	try {
		if (req.body.name === "") {
			...
			//[PK] This is a nice error message, but the user will never see it! It only appears in the server console. Generating these
			res.status(400)
		} else {
			...
			if (newFriend[1]) {
				...
				//[PK] ^I like what you've done here b/c you give the front end the chance to give an appropriate error to the user
				console.error(
					"You can't have two friends with the same name, dummy! Too confusing."
				)
			}
		}
	} catch (err) {
